...
function onLocationFound(event) {
  ...
  if (autoModeState.auto) {
    ...
    blinker.value.style.backgroundColor =
      bgcol != "red" ? "red" : "transparent";
    if (autoModeState.lastUpdateTime != null) {
      if (
        currentTime - autoModeState.lastUpdateTime <
        autoModeState.minInterval
      ) {
        return;
      } else {
        let r = geod.Inverse(
...
watch(
  ...
  (
    [newLatitude, newLongitude, newZoom],
    [oldLatitude, oldLongitude, oldZoom],
  ) => {
