<template>
  <div class="container fixed-top top-panel">
      <div class="row">
          <div class="col">
      ...
      <div class="row button-area m-0 my-2 d-flex flex-row justify-content-between align-items-center">
          <div class="col-8 d-flex flex-row justify-content-start align-items-center">
              <button v-on:click="onLocateButtonClick" class="btn btn-primary mx-0 px-4 py-1 text-nowrap" v-bind:disabled="trackButtonChecked">現在位置の取得</button>
              <button v-on:click="onRecordButtonClick" class="btn btn-primary mx-3 px-4 py-1 text-nowrap" v-bind:disabled="trackButtonChecked || !locationFoundEvent">記録</button>
          </div>
...
function setLatLngs(timestamp) {
  ...
  if (timestamp != null) {
    ...
    logs.value.filter((x) => timestamp0 <= x.timestamp && x.timestamp < timestamp24).forEach((x) => {
      ...
      logs.value = [
        {
          ...
          latitude: 35.6811124, longitude:139.764516, 
          address:'東京駅'
        }, 
        {
          timestamp: new Date().getTime() + 60000, 
          latitude: 36.5780486, longitude:136.6455965, 
          address:'金沢駅'
        }, 
      ];
      setLatLngs(null);
